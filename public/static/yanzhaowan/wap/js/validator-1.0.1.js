...
// 性别校验
VeeValidate.Validator.extend("xb", {
    getMessage: function () {
        return "考生性别选择不正确！";
    },
    validate: function (value, args) {
        var reg = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
        if (value && reg.test(args[0])) {
            var judegCode = '';
            if (args[0].length === 15) {
                judegCode = parseInt(args[0].substr(14, 1)) % 2;
            } else {
                judegCode = parseInt(args[0].substr(16, 1)) % 2;
            }
            if (judegCode === 1) {
                judegCode = '1';
            } else {
